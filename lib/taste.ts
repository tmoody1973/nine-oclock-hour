...
// Exported so the page's topic picker can list the same ten without redeclaring them.
export const ALL_TOPICS: Topic[] = ['news', 'politics', 'world', 'economy', 'health', 'tech', 'culture', 'climate', 'local', 'music'];
...
export function weightsFor(picks: Topic[]): ScoreWeights {
  return Object.fromEntries(ALL_TOPICS.map((t) => [t, picks.includes(t) ? 0 : 2]));
}
