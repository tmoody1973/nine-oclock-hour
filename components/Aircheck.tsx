...
export function Aircheck({ result, hour, day }: { result: ScoreResult; hour: Block[]; day: DayFile }) {
  ...
  return (
    <section aria-label="Aircheck">
      <div>Aircheck — <span className="figure">{total}/100</span></div>

      <div className="scores">
        {(Object.entries(result.scores) as [keyof typeof MAX, number][]).map(([label, value]) => (
          <div key={label} className="score">
            <div className="l">{label}</div>
            <div className="n figure">{value}<span>/{MAX[label]}</span></div>
          </div>
        ))}
      </div>
      ...
      <p>
        ...
        <span className="figure" role="img" aria-label={`Retention curve, bottoming at ${result.low}%`}>
      ...
      <div>
        <p>What the network actually did</p>
        <ul>
          {rundown.map((w) => (
            <li key={w.id}>{w.title} — <span className="figure">{clock(w.len || w.est || 0)}</span></li>
          ))}
        </ul>
        {missedTop && <p>Every other newsroom carried &ldquo;{day.mostCarried.title}&rdquo;. You didn&rsquo;t.</p>}
      </div>
    </section>
  );
}
